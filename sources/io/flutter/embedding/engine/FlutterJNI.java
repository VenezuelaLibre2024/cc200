...
public class FlutterJNI {
        ...
        if (Build.VERSION.SDK_INT >= 28) {
            try {
                return ImageDecoder.decodeBitmap(ImageDecoder.createSource(byteBuffer), new ImageDecoder.OnHeaderDecodedListener() { // from class: io.flutter.embedding.engine.a
                    ...
                    public /* synthetic */ C2735a(long j102) {
                        r1 = j102;
                    }

                    @Override // android.graphics.ImageDecoder.OnHeaderDecodedListener
                    public final void onHeaderDecoded(ImageDecoder imageDecoder, ImageDecoder.ImageInfo imageInfo, ImageDecoder.Source source) {
                        FlutterJNI.lambda$decodeImage$0(r1, imageDecoder, imageInfo, source);
                    }
                });
            } catch (IOException e10) {
                Log.m11042e(TAG, "Failed to decode image", e10);
            }
        }
        return null;
    }
    ...
    private native void nativeRegisterImageTexture(long j10, long j11, WeakReference<TextureRegistry.ImageTextureEntry> weakReference);
    ...
    public void dispatchEmptyPlatformMessage(String str, int i10) {
        ...
        Log.m11047w(TAG, "Tried to send a platform message to Flutter, but FlutterJNI was detached from native C++. Could not send. Channel: " + str + ". Response ID: " + i10);
    }
    ...
    public void dispatchPlatformMessage(String str, ByteBuffer byteBuffer, int i10, int i11) {
        ...
        Log.m11047w(TAG, "Tried to send a platform message to Flutter, but FlutterJNI was detached from native C++. Could not send. Channel: " + str + ". Response ID: " + i11);
    }
    ...
    public float getScaledFontSize(float f10, int i10) {
        ...
        Log.m11041e(TAG, "getScaledFontSize called with configurationId " + String.valueOf(i10) + ", which can't be found.");
        return -1.0f;
    }
    ...
    public void init(Context context, String[] strArr, String str, String str2, String str3, long j10) {
        if (initCalled) {
            Log.m11047w(TAG, "FlutterJNI.init called more than once");
        }
    ...
    public void invokePlatformMessageEmptyResponseCallback(int i10) {
        ...
        try {
            if (isAttached()) {
                ...
                Log.m11047w(TAG, "Tried to send a platform message response, but FlutterJNI was detached from native C++. Could not send. Response ID: " + i10);
            }
        } finally {
    ...
    public void loadLibrary() {
        if (loadLibraryCalled) {
            Log.m11047w(TAG, "FlutterJNI.loadLibrary called more than once");
        }
    ...
    public void prefetchDefaultFontManager() {
        if (prefetchDefaultFontManagerCalled) {
            Log.m11047w(TAG, "FlutterJNI.prefetchDefaultFontManager called more than once");
        }
        nativePrefetchDefaultFontManager();
        prefetchDefaultFontManagerCalled = true;
    }

    public void registerImageTexture(long j10, TextureRegistry.ImageTextureEntry imageTextureEntry) {
        ensureRunningOnMainThread();
        ensureAttachedToNative();
        nativeRegisterImageTexture(this.nativeShellHolderId.longValue(), j10, new WeakReference<>(imageTextureEntry));
    }
    ...
    public void requestDartDeferredLibrary(int i10) {
        ...
        if (deferredComponentManager != null) {
            ...
            Log.m11041e(TAG, "No DeferredComponentManager found. Android setup must be completed before using split AOT deferred components.");
        }
    }
